...
public class TextData
{
    ...
    public void SetMessageData(string message)
    {
        ...
        Time = Regex.Match(message, "\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2} ").Value;//单独提取时间
        Match QQName = Regex.Match(message, "[\u4E00-\u9FA5A-Za-z0-9_⑨]+\\([0-9]+\\)");
        ...
        string[] SingleConversation = Regex.Split(message, "\\r\\n", RegexOptions.IgnoreCase);
